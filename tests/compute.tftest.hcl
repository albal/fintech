# Isolation tests for the compute module.
# Verifies ALB and Lambda outputs are exposed, confirming ECS/Lambda resources
# were created with the expected wiring from network, security, and data modules.
...
run "compute_alb_and_lambdas_are_created" {
  command = apply

  assert {
    condition     = output.alb_dns_name != ""
    error_message = "compute module must expose alb_dns_name"
  }
}

run "compute_alb_present_when_name_changes" {
  command = apply
  ...
  assert {
    condition     = output.alb_dns_name != ""
    error_message = "compute module must expose alb_dns_name regardless of name/env"
  }
}
