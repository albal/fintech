# Isolation tests for the network module.
# Verifies VPC wiring, AZ scaling, and that outputs are non-empty regardless
# of az_count, since downstream modules depend on every network output.
...
run "network_outputs_present_default_azs" {
  command = apply

  assert {
    condition     = output.vpc_id != ""
    error_message = "network module must produce a vpc_id"
  }
}

run "network_outputs_present_single_az" {
  command = apply
  ...
  assert {
    condition     = output.vpc_id != ""
    error_message = "network module must produce vpc_id with az_count = 1"
  }
}

run "network_outputs_present_four_azs" {
  command = apply
  ...
  assert {
    condition     = output.vpc_id != ""
    error_message = "network module must produce vpc_id with az_count = 4"
  }
}
