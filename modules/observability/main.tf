...
data "aws_iam_policy_document" "ct_bucket_policy" {
  statement {
    ...
    condition {
      ...
      values   = ["arn:${data.aws_partition.current.partition}:cloudtrail:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:trail/${var.name}-trail"]
    }
  }
